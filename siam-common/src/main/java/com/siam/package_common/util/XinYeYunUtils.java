...
//        commonParams.put("debug", "1");
    ...
    public static JSONObject kitchenTotalOrderDataPrint(boolean isKitchenTotalOrder, JSONObject printingOrderInfo, JSONObject printingMenu,
                                                        List<JSONObject> printingMenuList) {
        ...
        if (!isKitchenTotalOrder) {
            String foodContent = backKitchenDataPrint(printingMenu);
            String goodsPrinterSn = printingMenu.getString("goodsPrinterSn");
            int printNum = printingMenu.getInteger("printNum");
            String goodsPrinterNames = printingMenu.getString("goodsPrinterNames");
            ...
            for (int i = 0; i < printNum; i++) {
                ...
                printResultData = print(goodsPrinterSn, content, "1", "2", "1");
            }
        }
        if (isKitchenTotalOrder) {
            String backKitchenFoodListContent = "";
            for (JSONObject food : printingMenuList) {
                String foodContent = backKitchenDataPrint(food);
            ...
            printResultData = print(backKitchentPrinterSn, backKitchenMenuContent, "1", "2", "1");
        }
     ...
     * 后厨单打印
     *
     * @param printingMenu 订单商品信息
     */
    public static String backKitchenDataPrint(JSONObject printingMenu) {
        String foodName = printingMenu.getString("foodName");
        String foodUnit = printingMenu.getString("foodUnit");
        int foodNum = printingMenu.getInteger("foodNum");
        String foodSpecs = printingMenu.getString("foodSpecs");

        String foodContent = "<TABLE col=\"22,18,8\" w=2 h=2><tr>" + foodName + "<td>" +
                foodSpecs + "<td>" + foodNum + foodUnit + "</tr></TABLE><BR>";
