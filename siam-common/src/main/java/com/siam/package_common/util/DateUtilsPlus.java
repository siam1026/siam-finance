...
public class DateUtilsPlus {
    ...
    public static List<String> getBetweenDays(String startTime, String endTime) {
        if (StringUtilsPlus.isEmpty(startTime) || StringUtilsPlus.isEmpty(endTime)) {
            return null;
        }
        //1、定义转换格式
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
    ...
    public static long timestampExample(String dateTime) {
        // 创建一个指定时间的Timestamp对象
    ...
    public static String OTCToLocalTime(String otcTimeString) {
        // 假设您有一个OTC时间的字符串，例如："2023-04-05T12:34:56Z"
        // 移除字符串末尾的'Z'来表示UTC时间
        String utcTimeString = otcTimeString.substring(0, otcTimeString.length() - 1);

        // 使用SimpleDateFormat来解析UTC时间
        SimpleDateFormat utcFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
        utcFormat.setTimeZone(TimeZone.getTimeZone("UTC"));

        Date utcDate = null;
        try {
            utcDate = utcFormat.parse(utcTimeString);
        } catch (ParseException e) {
            return null;
        }

        // 创建另一个SimpleDateFormat以按照您想要的时区格式化时间
        SimpleDateFormat localFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        localFormat.setTimeZone(TimeZone.getDefault()); // 默认为系统时区

        // 转换UTC时间为系统默认时区的时间
        String localTimeString = localFormat.format(utcDate);

        System.out.println("OTC Time: " + otcTimeString);
