...
export default {
	...
	methods: {
		changeIsOperating() {
			...
			let url = '/rest/member/shop/update';
		...
		passwordSubmit() {
			...
			this.$refs.passwordForm.validate((valid) => {
				if (valid) {
					...
					vue.$http.post(vue, '/rest/member/updatePassword', param,
						(vue, data) => {
							vue.logining = false;
							vue.$message({
								showClose: true,
		...
		logout: function () {
			...
			this.$confirm('确认退出吗?', '提示', {
				...
				vue.$http.post(vue, '/rest/member/logout', {},
					function (vue, data) {
	...
	mounted() {
		...
		// //获取开关设置
		// var vue = this;
		// vue.$http.post(vue, '/rest/member/shop/getLoginMemberShopInfo', {},
		// 	(vue, data) => {
		// 		let shop = data.data
