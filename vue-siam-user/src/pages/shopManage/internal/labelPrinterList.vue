	...
	export default {
		...
		methods: {
			...
			getList(pageNoParam) { // 获取列表
				...
				vue.$http.post(vue, '/rest/member/printer/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/member/printer/delete', {"ids" : [id]},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){					
							url = '/rest/member/printer/update';
							...
							//设置打印机类型为标签打印机
							param.type = 2;									
							url = '/rest/member/printer/insert';
