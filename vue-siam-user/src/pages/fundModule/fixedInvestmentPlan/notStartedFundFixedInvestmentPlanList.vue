	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchMsg">
				<el-form-item>
					<el-input v-model="searchMsg.fundName" clearable placeholder="请输入基金名称"></el-input>
				</el-form-item>
				<el-form-item>
					<el-select v-model="searchMsg.type" placeholder="请选择操作类型搜索" clearable>
						<el-option label="多基金混合定投" :value="0"></el-option>
						<el-option label="单基金定投" :value="1"></el-option>
					</el-select>
				</el-form-item>             
				<el-form-item>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column prop="type" label="操作类型">
				<template scope="scope">
					<span v-if="scope.row.type == 0">多基金混合定投</span>
                    <span v-if="scope.row.type == 1">单基金定投</span>
               </template>				
			</el-table-column>			
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" @close="closeDialog" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="150px" style="width: 80%;" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="基金名称" prop="fundIdComma">
					<el-select v-model="editForm.fundIdComma" class="minInput" filterable>
	...
	export default {
		data() {
			return {
				...
				editFormRules: {
					...
					fundIdComma: [{ required: true, message: '请选择基金名称', trigger: 'change' }],
		...
		methods: {
			...
			gotoOtherPage(type, row) {
				if(type === 'view') {
					this.$router.push({path:'viewFundFixedInvestmentPlan', query:{id: row.id}})
				}else if(type === 'viewSell') {
					this.$router.push({path:'viewFundFixedInvestmentPlanSell', query:{id: row.id}})
				}
			},
			...
			getList() { // 获取商品基金列表
				...
				vue.$http.post(vue, '/rest/member/fundFixedInvestmentPlan/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/member/fundFixedInvestmentPlan/delete', {"ids" : id},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			handleEdit: function (index, row) { // 显示编辑界面
				...
				if(row){
					this.editForm = {
						id: row.id,
						type: row.type,
						fundIdComma: parseInt(row.fundIdComma),
						amount: row.amount,
						cycle: row.cycle,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/member/fundFixedInvestmentPlan/update';						
						}else{
							url = '/rest/member/fundFixedInvestmentPlan/insert';						
						}
