	...
	<section>
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" @close="closeDialog" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="150px" style="width: 80%;" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="买入份额(份)" prop="shares">
					<el-input v-model="editForm.shares"></el-input>
				</el-form-item>
				<el-form-item label="买入净值" prop="netValue">
	...
	export default {
		data() {
			return {
				...
				editFormRules: {
					...
					shares: [{ required: true, message: '请输入买入份额', trigger: 'blur' }],
					netValue: [{ required: true, message: '请输入买入净值', trigger: 'blur' }],
		...
		methods: {
			...
			getList(id) { // 获取商品基金列表
				...
				vue.$http.post(vue, '/rest/member/fundFixedInvestment/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/member/fundFixedInvestment/delete', {"ids" : id},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/member/fundFixedInvestment/update';						
						}else{
							url = '/rest/member/fundFixedInvestment/insert';	
							param.type = 0;
