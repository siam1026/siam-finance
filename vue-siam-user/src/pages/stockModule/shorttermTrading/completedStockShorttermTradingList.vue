	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchMsg">
				...
				<!-- <el-form-item label="股票操作类型" prop="type">
	...
	export default {
		...
		methods: {
			...
			gotoOtherPage(type, row) {
				if(type === 'view') {
					this.$router.push({path:'viewStockShorttermTrading', query:{id: row.id}})
				}
			},
			...
			getList() { // 获取商品股票列表
				...
				vue.$http.post(vue, '/rest/member/stockShorttermTrading/list', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/member/stockShorttermTrading/delete', {"ids" : id},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/member/stockShorttermTrading/update';						
						}else{
							url = '/rest/member/stockShorttermTrading/insert';						
						}
