	...
	export default {
		...
		methods: {
			...
			getList() { // 获取商品列表
				...
				vue.$http.post(vue, '/rest/member/stock/statistics', param,
					(vue, data) => {
					...
					for(var i = 0; i < result.shortTermList.length; i++){
						var map = {};
						map.name = result.shortTermList[i].stockName + "(" + result.shortTermList[i].ratio + "%)";
					...
					var map = {};
						map.name = result.longTermList[i].stockName + "(" + result.longTermList[i].ratio + "%)";
