<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchMsg">
				<el-form-item label="" prop="name">
					<el-input v-model="searchMsg.name" clearable placeholder="请输入股票名称"></el-input>
				</el-form-item>
				<el-form-item label="" prop="code">
					<el-input v-model="searchMsg.code" clearable placeholder="请输入股票代码"></el-input>
				</el-form-item>									
				<!-- <el-form-item label="股票状态" prop="disabled">
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column prop="market" label="市场简写"></el-table-column>
			<el-table-column prop="state" label="上市状态"></el-table-column>
			<el-table-column prop="currcapital" label="流通股本(万股)"></el-table-column>
			<el-table-column prop="profitFour" label="四季度净利润(亿元)"></el-table-column>
			<el-table-column prop="listingDate" label="上市日期"></el-table-column>
			<el-table-column prop="totalcapital" label="总股本(万股)"></el-table-column>
	...
	export default {
		data() {
			return {
				...
				editLoading: false,
				editFormRules: {
					// disabled: [{ required: true, message: '请选择股票状态', trigger: 'change' }],
		...
		methods: {
			...
			syncPrice: function (index, row) { // 同步最新价格
				let vue = this;
				vue.$http.post(vue, '/rest/member/stock/syncPrice', {"isOptional" : true},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			handleDel (id) { // 删除
				...
				vue.$http.post(vue, '/rest/member/stock/delete', {"id" : id},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/member/stock/update';					
						}else{
							url = '/rest/member/stock/insert';						
						}
