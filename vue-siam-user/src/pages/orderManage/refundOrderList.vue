<template>
	<section>
      ...
      <el-table-column prop="refundReason" label="退款原因">
          ...
          <span v-if="scope.row.refundReason == 4">用户缺货</span>
          <span v-if="scope.row.refundReason == 5">用户联系我取消</span>
          ...
          <span v-if="scope.row.refundReason == 9">用户通知我卖完了</span>
          <span v-if="scope.row.refundReason == 10">用户沟通态度差</span>
          ...
          <span v-if="scope.row.refundReason == 14">用户少送商品</span>
          <span v-if="scope.row.refundReason == 15">用户送错商品</span>
      ...
      <el-table-column prop="refundStatus" label="退款状态">
          ...
          <span v-if="scope.row.refundStatus == 2">等待用户处理</span>
          <span v-if="scope.row.refundStatus == 3">用户拒绝退款</span>
	...
	export default {
      ...
      handleCheck(row){
        ...
        let vue = this;
				...
				let url = '/rest/member/order/auditAfterSalesOrder';
      ...
      dealOption() {
        ...
        vue.dealtype ? url = '/rest/member/order/updateOrderByBack' : url = '/rest/member/order/updateByDealadvise'
        vue.$http.post( vue, url, param,
          (vue, data) => {
            vue.$message({
              showClose: true,
      ...
      addStand() { // 新增规格
        ...
        vue.$http.post( vue, '/rest/member/stand/update', param,
          (vue, data) => {
            vue.$message({
              showClose: true,
              message: data.message,
      ...
      getList(pageNoParam) { // 获取列表
        ...
        vue.listLoading = true;
				vue.$http.post(vue, '/rest/member/order/afterSalesList', param,
					(vue, data) => {
			...
			handleDel (id) { // 删除
				...
				let vue = this;
				vue.$http.post(vue, '/rest/member/stand/deleteById', {id},
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
        ...
        let param = {
          ...
          status: 1
				...
				vue.$http.post(vue, '/rest/member/order/updateOrderState', param,
					function(vue, data) {
            vue.$message({
              showClose: true,
              message: data.message,
