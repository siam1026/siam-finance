	...
	methods: {
		...
		xpYunPrinterOrderDetail() {
			...
			this.$confirm('是否打印当前订单结账单?', '提示', {
				...
				vue.$http.post(vue, '/rest/member/order/xpYunPrinterOrderDetail', { "pageNo": -1, "pageSize": 20, "id": id },
					(vue, data) => {
						console.log(data)
						loading.close();
						this.$message({
		...
		printOrderDetail() {
			...
			//获取订单信息
			let vue = this
			vue.$http.post(vue, '/rest/member/order/selectById', { "id": id },
				(vue, data) => {
					let order = data.data;
					...
					//获取订单明细信息
					vue.$http.post(vue, '/rest/member/orderDetail/list', { "pageNo": -1, "pageSize": 20, "orderId": id },
						(vue, data) => {
		...
		getRelatedList(id) { // 获取商品列表
			...
			vue.$http.post(vue, '/rest/member/order/list', param,
				(vue, data) => {
		...
		getList(id) { // 获取商品列表
			...
			vue.$http.post(vue, '/rest/member/orderDetail/list', param,
				(vue, data) => {
		...
		handleDel(id) { // 删除
			this.$confirm('确认删除该记录吗?', '提示', {
				...
				vue.$http.delete(vue, '/rest/member/orderDetail/delete', { "ids": [id] },
					function (vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
		...
		editSubmit: function () { // 编辑
			this.$refs.editForm.validate((valid) => {
				if (valid) {
					...
					let url = '';
					if (param.id) {
						url = '/rest/member/orderDetail/update';
					} else {
						url = '/rest/member/orderDetail/insert';
