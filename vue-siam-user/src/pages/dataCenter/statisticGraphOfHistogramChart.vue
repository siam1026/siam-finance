<template>
  <div class="app-container">    
    ...
    <el-form :inline="true">
      ...
      <el-form-item label="" prop="">
        <el-select v-model="sortType" clearable placeholder="请选择排序方式" @change="handleCapitalTypeIdChange">
          <el-option label="收入从高到低" :value="0"></el-option>
          <el-option label="收入从低到高" :value="1"></el-option>
          <el-option label="支出从高到低" :value="2"></el-option>
          <el-option label="支出从低到高" :value="3"></el-option>
        </el-select>
      </el-form-item>
      
      <el-form-item label="" prop="">
        <el-select v-model="showType" clearable placeholder="请选择展示方式" @change="handleCapitalTypeIdChange">
          <el-option label="只展示收入大于0的数据" :value="0"></el-option>
          <el-option label="只展示支出大于0的数据" :value="1"></el-option>
        </el-select>
      </el-form-item>
    ...
    <div class="statistics-layout">
      <div class="layout-title-income">资金统计</div>
      <el-row>
        <el-col :span="4">
          <div style="padding: 20px">
            ...
            <div>
              <div style="color: #909399;font-size: 14px">总结余（元）</div>
              <div style="color: red;font-size: 24px;padding: 10px 0" v-if="todayList.totalOrderAmountOfLeave >= 0">{{todayList.totalOrderAmountOfLeave | formatMoney}}</div>
              <div style="color: green;font-size: 24px;padding: 10px 0" v-else>{{todayList.totalOrderAmountOfLeave | formatMoney}}</div>
            </div>            
          </div>
        </el-col>
        <el-col :span="20">
    ...
    data() {
      return {
        ...
        chartSettings: {
          ...
          labelMap: {'type': '类别', 'income': '收入(元)', 'expend': '支出(元)', 'leave': '结余(元)'},   
        },
        chartExtend: {
          // x轴的文字倾斜(范围：-90~90)
        ...
        chartDataOfIncome: {
          columns: ['type', 'income', 'expend', 'leave'],
          rows: []
        },
    ...
    methods:{
      ...
      getData(){
          ...
          setTimeout(() => {
            ...
            if(this.sortType == 0){
              params.sortKey = "income";
              params.sortType = "desc";
            }else if(this.sortType == 1){
              params.sortKey = "income";
              params.sortType = "asc";
            }else if(this.sortType == 2){
              params.sortKey = "expend";
              params.sortType = "desc";
            ...
            vue.$http.post(vue, '/rest/member/capitalRecord/statisticsOfHistogramChart', params,
              (vue, data) => {
                //加载折线图数据
                this.chartDataOfIncome.rows = data.data.list;
                //渲染交易总数、交易总额
                this.todayList.totalOrderAmountOfIncome = data.data.totalIncome;
                this.todayList.totalOrderAmountOfExpend = data.data.totalExpend;
                this.todayList.totalOrderAmountOfLeave = data.data.totalLeave;
