			...
			<el-form :inline="true" :model="searchMsg" size="small">
				...
				<el-form-item>
					<el-select v-model="searchMsg.type" placeholder="请选择账单类型搜索" clearable>
						...
						<el-option label="用户提现" :value="2"></el-option>    
						<el-option label="用户提现失败退回" :value="3"></el-option>
						<el-option label="用户自配送-配送费收入" :value="6"></el-option>
	...
	export default {
		...
		methods: {
			...
			getList(pageNoParam) { // 获取列表
				...
				vue.$http.post(vue, '/rest/member/merchantBillingRecord/list',param,
					(vue, data) => {
				...
				vue.$http.post(vue, '/rest/member/merchantBillingRecord/statisticalAmount', param,
					(vue, data) => {
						//统计入账、出账的金额						
