...
<script>
  ...
  export default {
    ...
    methods: {    
      saveShopMsg() {
        this.$refs.editForm.validate((valid) => {
          if (valid) {
            ...
            let message = '';
            //计算平台服务费
            let actualMemberWithdrawFeeRatio = (Number(merchantWithdrawFeeRatio) / 100).toFixed(2);
            let platformFee = (Number(param.withdrawAmount) * Number(actualMemberWithdrawFeeRatio)).toFixed(2);
            let totalAmount = (Number(param.withdrawAmount) + Number(platformFee)).toFixed(2);   
            // alert(merchantWithdrawFeeRatio + "-" + actualMemberWithdrawFeeRatio + "-" + platformFee + "-" + totalAmount);
      ...
      getDetail() { // 获取商品详情
          let vue = this
          vue.$http.post(vue, '/rest/member/getLoginMemberInfo', {},
            (vue, data) => {
              let obj = data.data
