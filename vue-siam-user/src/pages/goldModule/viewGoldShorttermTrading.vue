<template>
	<section>
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column prop="grams" label="卖出克数(份)">
				<template scope="scope">
					<span>{{changeDecimal(scope.row.grams, 2)}}</span>
               </template>					
			</el-table-column>
			<el-table-column prop="price" label="卖出金价">
				<template scope="scope">
					<span>{{changeDecimal(scope.row.price, 4)}}</span>
               </template>					
			</el-table-column>
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" @close="closeDialog" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="150px" style="width: 80%;" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="卖出克数(份)" prop="grams">
					<el-input v-model="editForm.grams"></el-input>
				</el-form-item>
				<el-form-item label="卖出金价" prop="price">
		...
		data() {
			return {
				...
				editFormRules: {
					...
					price: [{ required: true, message: '请输入卖出金价', trigger: 'blur' }],
