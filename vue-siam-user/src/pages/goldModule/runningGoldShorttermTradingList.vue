	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="searchMsg">
				<el-form-item label="" prop="goldName">
					<el-input v-model="searchMsg.goldName" clearable placeholder="请输入黄金名称"></el-input>
				</el-form-item>
				<!-- <el-form-item label="黄金操作类型" prop="type">
		...
		<el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" :cell-style="cellStyle" :header-cell-style="headerCellStyle">
			...
			<el-table-column prop="goldName" label="黄金名称"></el-table-column>
			...
			<el-table-column prop="grams" label="买入克数(份)">
				<template scope="scope">
					<span>{{changeDecimal(scope.row.grams, 2)}}</span>
               </template>					
			</el-table-column>
			<el-table-column prop="price" label="买入金价">
				<template scope="scope">
					<span>{{changeDecimal(scope.row.price, 4)}}</span>
               </template>					
			</el-table-column>
			<el-table-column prop="remainingGrams" label="剩余克数(份)">
				<template scope="scope">
					<span v-if="scope.row.remainingGramsRate == 100">{{changeDecimal(scope.row.remainingGrams, 2)}}<br/>（{{scope.row.remainingGramsRate}}%）</span>
                    <span v-else style="color: black; font-weight: bold">{{changeDecimal(scope.row.remainingGrams, 2)}}<br/>（{{changeDecimal(scope.row.remainingGramsRate, 2)}}%）</span>
               </template>					
			</el-table-column>
			...
			<el-table-column label="操作" fixed="right" width="150">
				<template slot-scope="scope">
					...
					<span v-if="scope.row.remainingGramsRate != 1100">
		...
		<el-dialog title="编辑" :visible.sync="editFormVisible" @close="closeDialog" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="150px" style="width: 80%;" :rules="editFormRules" ref="editForm">
				<el-form-item label="黄金名称" prop="goldId">
					<el-select v-model="editForm.goldId" class="minInput">
						<el-option v-for="item in goldList" :key="item.id" :label="item.name" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="买入克数(份)" prop="grams">
					<el-input v-model="editForm.grams"></el-input>
				</el-form-item>
				<el-form-item label="买入金价" prop="price">
	...
	export default {
		data() {
			return {
				...
				editLoading: false,
				editFormRules: {
					goldId: [{ required: true, message: '请选择黄金名称', trigger: 'change' }],
					amount: [{ required: true, message: '请输入买入金额', trigger: 'change' }],
					grams: [{ required: true, message: '请输入买入克数', trigger: 'blur' }],
					price: [{ required: true, message: '请输入买入金价', trigger: 'blur' }],
		...
		methods: {
			...
			gotoOtherPage(type, row) {
				if(type === 'view') {
					this.$router.push({path:'viewGoldShorttermTrading', query:{id: row.id}})
				}
			},
			...
			getList() { // 获取商品黄金列表
				...
				vue.$http.post(vue, '/rest/member/goldShorttermTrading/list', param,
					(vue, data) => {
			...
			getTypeList() { // 获取商品类别列表
				...
				vue.$http.post(vue, '/rest/member/gold/list', param,
					(vue, data) => {
						vue.goldList = data.data.records
					},(error, data)=> {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			handleDel (id) { // 删除
				...
				vue.$http.delete(vue, '/rest/member/goldShorttermTrading/delete', {"ids" : id},
					function(vue, data) {
						vue.$message({
							showClose: true,
							message: data.message,
			...
			editSubmit: function () { // 编辑
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						...
						let url = '';
						if(param.id){
							url = '/rest/member/goldShorttermTrading/update';						
						}else{
							url = '/rest/member/goldShorttermTrading/insert';	
							param.type = 0;					
						}
